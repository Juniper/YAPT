...
function prepend_device_in_table(device){
    ...
    device.deviceTasksStates.forEach(function (value, i) {
        if (typeof value[1] === "object") {
            tempRow = "<tr id=" + device.deviceTaskSequence[i] + "><td>" + device.deviceTaskSequence[i] + "</td>" + "<td class=\"col-md-8\" colspan=\"2\">" + value[1]['taskStateMsg'] +
            ...
            tempRow = "<tr id=" + device.deviceTaskSequence[i] + "><td>" + device.deviceTaskSequence[i] + "</td>" + "<td class=\"col-md-8\" colspan=\"2\">" + value[1] +
            "<td><button id=\"configDetail" + device.deviceSerial + "\" type=\"button\" class=\"btn btn-default btn-sm glyphicon glyphicon-eye-open\" data-toggle=\"modal\" data-target=\"#modal" + device.deviceSerial + "\"></button>" +
            "</td></tr>";
        }
                            ...
                            "<table class=\"table table-hover\" id=\"task" + device.deviceSerial + "\">" +
                                "<thead>" +
                                    "<tr>" +
                                        ...
                                        "<td>Service Plugin: " + device.deviceServicePlugin + "</td>" +
...
function mainNavbarEventHandler(){
    ...
    $(".mainNav").on('click', function (e) {
        ...
        } else if (hash.attr("href") === "#config-validation") {
...
function initialize(){
    ...
    valSaveBtnEventHandler();
    valDelBtnEventHandler();
}
...
function initLogViewerData(data) {
    $("#mainlogviewer").val(data);
    $("#mainlogviewer").text(data);
}
