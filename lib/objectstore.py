...
class ObjectView(object):
    ...
    def __init__(self, entries, default=False):
        ...
        AttributeDict.test would return "test" instead of raising an error,
        if no .test attribute were found.
        """
    ...
    def add_entries(self, entries, default=False):
        for key, value in entries.items():
            if type(value) is dict:
                self.__dict__[key] = ObjectView(value, default)
            else:
                try:
                    ...
                    # if value is an internal, expandvars() will fail; if
